...
import { Home, ChevronRight, Package, Box, CheckCircle2, AlertCircle, Info, XCircle, RefreshCw, ArrowLeftRight, Archive, Search, Check, ArrowLeft, Layers } from "lucide-react";
...
// Generate random layer count (1-10)
const generateRandomLayers = () => Math.floor(Math.random() * 10) + 1;

// Mock data for pallet
const generateMockPalletData = () => {
 const layers = generateRandomLayers();
 const casesPerLayer = 3;
 return {
 palletId: "PLT-12345",
 location: "R102S03P01",
 sku: "SKU-4521",
 ...
 totalLayers: layers,
 totalCases: layers * casesPerLayer,
 layersToDeLayer: layers,
 casesPerLayer: casesPerLayer,
 status: "Ready for Manual DeLayer",
 exceptionReason: "Partial Layer"
 };
};

const mockPalletData = generateMockPalletData();
...
const generatePalletGrid = (totalCases: number, casesPerLayer: number, layersToDeLayer: number) => {
 ...
 layer: layer + 1,
 position: position + 1,
 isWrapped: layer < layersToDeLayer,
 caseNumber
 });
 }
 }
...
export function DeLayer() {
 ...
 const [deWrapComplete, setDeLayerComplete] = useState(false);
 ...
 const [showTGWTray, setShowTGWTray] = useState(false);
 const [tgwTrayID, setTgwTrayID] = useState("");
 const [pickedCases, setPickedCases] = useState(0);
 const [pickedLayers, setPickedLayers] = useState(0);
 const [currentTrayNumber, setCurrentTrayNumber] = useState(1);
 ...
 // For DeLayer, skip layer entry and go straight to confirmed state with pallet visualization
 // Generate cases grid based on pallet data
 const grid = generatePalletGrid(
 palletData.totalCases,
 palletData.casesPerLayer,
 palletData.layersToDeLayer
 );
 setCasesGrid(grid);
 ...
 description: "Ready to begin delayer",
 duration: 3000,
 });
 }
 };
 ...
 const handleDeLayerComplete = () => {
 // Generate a new tray ID automatically and show TGW tray immediately
 const newTrayID = `TGW-${String(currentTrayNumber).padStart(4, '0')}`;
 setTgwTrayID(newTrayID);
 setShowTGWTray(true);

 // Show toast notification
 toast.success("TGW Tray Arrived", {
 description: `Tray ${newTrayID} has arrived`,
 duration: 3000,
 });
 };

 const handleCasePickComplete = () => {
 // Increment picked cases
 const newPickedCases = pickedCases + 1;
 setPickedCases(newPickedCases);

 toast.success("Case picked", {
 description: `Tray ID: ${tgwTrayID}`,
 duration: 2000,
 });

 // Generate next tray ID and stay on TGW screen
 setCurrentTrayNumber(prev => prev + 1);
 const nextTrayID = `TGW-${String(currentTrayNumber + 1).padStart(4, '0')}`;
 setTgwTrayID(nextTrayID);

 // Show toast for next tray arrival
 toast.success("TGW Tray Arrived", {
 description: `Tray ${nextTrayID} has arrived`,
 duration: 2000,
 });
 };

 const handleLayerPickComplete = () => {
 // Increment picked layers
 const newPickedLayers = pickedLayers + 1;
 setPickedLayers(newPickedLayers);

 toast.success("Layer complete", {
 description: `Layer ${newPickedLayers} of ${palletData.layersToDeLayer} completed`,
 duration: 2000,
 });

 // Check if all layers have been picked
 if (newPickedLayers >= palletData.layersToDeLayer) {
 // All picks complete - close TGW screen and show completion screen
 setShowTGWTray(false);
 setDeLayerComplete(true);
 } else {
 // More layers to go - generate next tray ID and show toast
 setCurrentTrayNumber(prev => prev + 1);
 const nextTrayID = `TGW-${String(currentTrayNumber + 1).padStart(4, '0')}`;
 setTgwTrayID(nextTrayID);

 toast.success("TGW Tray Arrived", {
 description: `Tray ${nextTrayID} has arrived - Layer ${newPickedLayers + 1}`,
 duration: 3000,
 });
 }
 };

 const handleCancelTGWTray = () => {
 setShowTGWTray(false);
 setTgwTrayID("");
 };
 ...
 setShowTGWTray(false);
 setTgwTrayID("");
 setCurrentTrayNumber(1);

 toast.success("Manual DeLayer operation completed", {
 ...
 setTimeout(() => {
 // Generate new pallet data with random layers
 const newPalletData = generateMockPalletData();
 ...
 toast.success(`${newPalletData.palletId} has arrived`, {
 description: `Location: ${newPalletData.location}`,
 duration: 4000,
 });
 }
 }, 5000);
 ...
 <div>
 <label className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider mb-1 block">Layers to DeLayer</label>
 <p className="text-[var(--foreground)] font-semibold text-sm">{palletData.layersToDeLayer}</p>
 </div>
...
{layersConfirmed && (
 ...
 <span className="text-sm text-[var(--muted-foreground)] dark:text-[var(--muted-foreground)]">Layers to DeLayer</span>
 </div>
 <div className="flex items-center gap-2">
...
{/* Pallet Grid, Confirmation, or Keypad */}
 ...
 <div className="space-y-4 mb-6">
 <div className="bg-[var(--state-warning)]/10 border border-[var(--state-warning)]/40 rounded-lg p-4">
 <label className="text-xs text-[var(--state-warning)] uppercase tracking-wider mb-1 block">Exception Reason</label>
 <p className="text-[var(--state-warning)] font-semibold text-lg">{palletData.exceptionReason}</p>
 </div>
 ...
 <h3 className="text-xl font-semibold text-[var(--foreground)]  mb-2">Place Case on TGW Tray</h3>
 <p className="text-sm text-[var(--muted-foreground)] dark:text-[var(--muted-foreground)]">Use this tray ID for the current case</p>
 </div>

 <div className="space-y-4 mb-6">
 <div className="bg-[var(--state-info)]/10 border-2 border-[var(--state-info)]/40 rounded-lg p-6 text-center">
 <label className="text-xs text-[var(--state-info)] uppercase tracking-wider mb-2 block">TGW Tray ID</label>
 <p className="text-[var(--foreground)] font-mono font-bold text-3xl">{tgwTrayID}</p>
 </div>

 <div className="bg-[var(--surface-container)] text-[var(--foreground)] border-[var(--border)] rounded-lg p-4">
 <label className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider mb-1 block">Progress</label>
 <div className="grid grid-cols-2 gap-4 mt-2">
 <div>
 <p className="text-xs text-[var(--muted-foreground)] dark:text-[var(--muted-foreground)] mb-1">Layers</p>
 <p className="text-[var(--foreground)] font-semibold text-lg">
 {pickedLayers} / {palletData.layersToDeLayer}
 </p>
 </div>
 <div>
 <p className="text-xs text-[var(--muted-foreground)] dark:text-[var(--muted-foreground)] mb-1">Cases</p>
 <p className="text-[var(--foreground)] font-semibold text-lg">{pickedCases}</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="space-y-3">
 <button
 onClick={handleCasePickComplete}
 className="w-full px-6 py-4 bg-[var(--primary)] hover:opacity-90 active:scale-[0.98] text-[var(--primary-foreground)] rounded-lg font-semibold transition-colors flex items-center justify-center gap-2"
 >
 <CheckCircle2 size={20} />
 Case Pick Complete
 ...
 <button
 onClick={handleLayerPickComplete}
 className="w-full px-6 py-4 bg-[var(--state-info-container)]/80 hover:bg-[var(--state-info)] text-[var(--state-on-info-container)] rounded-lg font-semibold transition-colors flex items-center justify-center gap-2"
 >
 <CheckCircle2 size={20} />
 Layer Complete
 </button>
 ...
 <h3 className="text-xl font-semibold text-[var(--foreground)]  mb-2">Confirm Manual DeLayer Completion</h3>
 ...
 <div className="bg-[var(--surface-container)] text-[var(--foreground)] border-[var(--border)] rounded-lg p-4">
 <label className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider mb-1 block">Cases Picked</label>
 <p className="text-[var(--state-success)] font-semibold text-3xl">{pickedCases}</p>
 </div>

 <div className="bg-[var(--surface-container)] text-[var(--foreground)] border-[var(--border)] rounded-lg p-4">
 <label className="text-xs text-[var(--muted-foreground)] uppercase tracking-wider mb-1 block">Layers Completed</label>
 <p className="text-[var(--foreground)] font-semibold text-lg">{pickedLayers}</p>
 </div>
 </div>
 ...
 <div className="flex flex-col gap-3 p-6 bg-[var(--surface-container)] text-[var(--foreground)] border-[var(--border)] rounded-xl border-[var(--border)]  overflow-y-auto max-h-full">
 {Array.from({ length: palletData.totalLayers }, (_, index) => {
 const layerNumber = index + 1; // Count from top: 1, 2, 3...
 const isLayerToDeLayer = layerNumber <= palletData.layersToDeLayer;
 ...
 <p className={`text-2xl font-bold ${isLayerToDeLayer ? 'text-[var(--state-success)]' : 'text-[var(--muted-foreground)]'}`}>
 ...
 <>
 {/* DeLayer Instructions */}
 <div className="bg-[var(--surface-container-high)] text-[var(--foreground)] border-[var(--border)]  rounded-xl p-4 flex-shrink-0">
 <div className="flex items-center gap-2 mb-4">
 <div className="w-8 h-8 bg-[var(--state-info)]/20 rounded-lg flex items-center justify-center">
 <Info size={16} className="text-[var(--state-info)]" />
 </div>
 <h2 className="font-semibold text-[var(--foreground)] ">Manual DeLayer Instructions</h2>
 </div>

 <div className="space-y-3">
 <div className="grid grid-cols-2 gap-2">
 <div className="bg-[var(--state-info)]/10 border border-[var(--state-info)]/40 rounded-lg p-2">
 <p className="text-xs text-[var(--state-info)] mb-1">Cases Picked</p>
 <p className="text-xl font-bold text-[var(--state-info)]">{pickedCases}</p>
 </div>
 <div className="bg-[var(--state-success-container)]/60 border border-[var(--state-success)]/30 rounded-lg p-2">
 <p className="text-xs text-[var(--state-success)] mb-1">Layers to Pick</p>
 <p className="text-xl font-bold text-[var(--state-success)]">{palletData.layersToDeLayer}</p>
 </div>
 </div>
 ...
 <div className="flex items-start gap-2">
 <CheckCircle2 size={14} className="text-[var(--state-success)] mt-0.5 flex-shrink-0" />
 <p className="text-xs text-[var(--foreground)]">Pick case from pallet</p>
 </div>
 <div className="flex items-start gap-2">
 <CheckCircle2 size={14} className="text-[var(--state-success)] mt-0.5 flex-shrink-0" />
 <p className="text-xs text-[var(--foreground)]">Scan TGW Tray ID</p>
 </div>
 <div className="flex items-start gap-2">
 <CheckCircle2 size={14} className="text-[var(--state-success)] mt-0.5 flex-shrink-0" />
 <p className="text-xs text-[var(--foreground)]">Place case on TGW tray</p>
 </div>
 </div>

 <button
 onClick={handleDeLayerComplete}
 className="w-full px-4 py-3 bg-[var(--primary)] hover:opacity-90 active:scale-[0.98] text-[var(--primary-foreground)] rounded-lg font-semibold transition-colors flex items-center justify-center gap-2"
 >
 <CheckCircle2 size={18} />
 Pick Cases
