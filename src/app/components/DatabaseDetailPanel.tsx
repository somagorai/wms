import { X, Database, HardDrive, Activity, Zap, TrendingUp, Clock, Server, AlertCircle, ChevronDown, ChevronUp, Table, CheckCircle2, Info, History, Search } from "lucide-react";
...
export function DatabaseDetailPanel({ dbName, dbType, computerName, dataType, onClose }: DatabaseDetailPanelProps) {
 ...
 setExpandedEvents(newExpanded);
  ...
  const tabs = [
    ...
    { id: "storage", label: isRabbitMQ ? "Queues" : "Storage", icon: <HardDrive size={16} /> },
    ...(!isRabbitMQ ? [{ id: "queries", label: "Queries", icon: <Search size={16} /> }] : []),
  ...
  return (
    <DetailSidePanel
      title={computerName}
      subtitle={`${dbType} Environment (${dataType})`}
      icon={<Database size={24} className="text-[var(--primary)]" />}
    ...
    >
      {activeTab === "details" ? (
        <div className="space-y-6">
          {/* Connection Section */}
          <PanelSection title="Connection Information">
            <PanelRow label="Host" value={details.connection.host} mono />
            <PanelRow label="Port" value={details.connection.port} mono />
            {isRabbitMQ && (
              <PanelRow label="Management Port" value={(details.connection as any).managementPort} mono />
            )}
            <PanelRow
              label="Status"
              value={<span className="text-sm font-medium text-[var(--state-success)]">{details.connection.status}</span>}
            />
            <PanelRow label="Uptime" value={details.connection.uptime} mono />
          </PanelSection>

          {/* Configuration Section */}
          <PanelSection title="Configuration">
            <PanelRow label="Version" value={details.configuration.version} mono />
            <PanelRow label="Edition" value={details.configuration.edition} />
            {isRabbitMQ ? (
              <>
                <PanelRow label="Erlang Version" value={(details.configuration as any).erlangVersion} mono />
                <PanelRow label="Cluster Mode" value={(details.configuration as any).clusterMode} mono />
              </>
            ) : (
              <>
                <PanelRow label="Character Set" value={details.configuration.charset} mono />
                <PanelRow label="Collation" value={details.configuration.collation} mono />
              </>
            )}
            <PanelRow label="Max Connections" value={details.configuration.maxConnections} mono />
            <PanelRow label="Current Connections" value={details.configuration.currentConnections} mono />
          </PanelSection>
          ...
          <PanelSection title="Performance Metrics">
            {isRabbitMQ ? (
              <>
                <PanelRow label="Avg Latency" value={(details.performance as any).avgLatency} mono />
                <PanelRow label="Messages/min" value={(details.performance as any).messagesPerMin} mono />
                <PanelRow label="Publish Rate" value={(details.performance as any).publishRate} mono />
                <PanelRow label="Delivery Rate" value={(details.performance as any).deliveryRate} mono />
                <PanelRow label="Ack Rate" value={(details.performance as any).ackRate} mono />
              </>
            ) : (
              <>
                <PanelRow label="Avg Query Time" value={(details.performance as any).avgQueryTime} mono />
                <PanelRow label="Transactions" value={(details.performance as any).transactions} mono />
                <PanelRow label="Cache Hit Ratio" value={(details.performance as any).cacheHitRatio} mono />
                <PanelRow label="I/O Throughput" value={(details.performance as any).ioThroughput} mono />
              </>
            )}
          </PanelSection>

          {/* Resources / Storage Section */}
          {isRabbitMQ ? (
            <PanelSection title="Resources">
              <PanelRow label="Memory Used" value={(details as any).resources.memoryUsed} mono />
              <PanelRow label="Memory Limit" value={(details as any).resources.memoryLimit} mono />
              <PanelRow label="Disk Free" value={(details as any).resources.diskFree} mono />
              <PanelRow label="File Descriptors" value={(details as any).resources.fileDescriptors} mono />
              <PanelRow label="Socket Descriptors" value={(details as any).resources.socketDescriptors} mono />
            </PanelSection>
          ) : (
            <PanelSection title="Storage">
              <PanelRow label="Total Size" value={(details as any).storage.totalSize} mono />
              <PanelRow label="Data Size" value={(details as any).storage.dataSize} mono />
              <PanelRow label="Index Size" value={(details as any).storage.indexSize} mono />
              <PanelRow label="Log Size" value={(details as any).storage.logSize} mono />
              <PanelRow label="Growth Rate" value={(details as any).storage.growth} mono />
            </PanelSection>
          )}
        </div>
 ) : activeTab === "storage" ? (
 ...
 <HardDrive size={18} className="text-[var(--primary)] dark:text-[var(--primary)]" />
 <p className="text-sm text-[var(--muted-foreground)]">
 {isRabbitMQ ? "Queue Metrics" : "Storage Metrics"} • <span className="font-semibold text-[var(--foreground)] ">{dbType} Environment</span>
 </p>
 </div>
 </div>
 ...
 <div
 key={queue.name}
 className="bg-[var(--surface-container)] text-[var(--foreground)] border border border-[var(--border)]  rounded-xl overflow-hidden"
 >
 <div className="p-4">
 <div className="flex items-center justify-between mb-3">
 <div className="flex items-center gap-2">
 <Database size={16} className="text-[var(--primary)] dark:text-[var(--primary)]" />
 <span className="font-mono text-sm font-semibold text-[var(--foreground)] ">{queue.name}</span>
 </div>
 ...
 : "bg-[var(--surface-container-low)] dark:bg-[var(--card)] text-[var(--muted-foreground)]"
 }`}>
 {queue.state}
 </span>
 </div>
 ...
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Messages Ready</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">
 {queue.messagesReady.toLocaleString()}
 </div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Unacknowledged</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">
 {queue.messagesUnacknowledged.toLocaleString()}
 </div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Total Messages</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">
 {queue.totalMessages.toLocaleString()}
 </div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Consumers</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">
 {queue.consumers}
 </div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Publish Rate</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">
 {queue.publishRate}
 </div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Delivery Rate</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">
 {queue.deliveryRate}
 </div>
 </div>
 ...
 <div
 key={db.name}
 className="bg-[var(--surface-container)] text-[var(--foreground)] border border border-[var(--border)]  rounded-xl overflow-hidden hover:border-[var(--primary)] dark:hover:border-[var(--primary)] transition-colors"
 >
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] px-4 py-3 border-b border-[var(--border)]  flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-lg bg-[var(--primary)]/20 /20 flex items-center justify-center">
 <span className="text-sm font-bold text-[var(--primary)] dark:text-[var(--primary)]">{db.name}</span>
 </div>
 <div>
 <div className="flex items-center gap-2">
 <HardDrive size={16} className="text-[var(--state-warning)]" />
 <span className="font-mono text-sm font-semibold text-[var(--state-warning)]">{db.size}</span>
 </div>
 <span className="text-xs text-[var(--muted-foreground)]">{db.growthRate}</span>
 </div>
 </div>
 <button
 ...
 <div className="grid grid-cols-2 gap-3">
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Used</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">{db.used}</div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Unused</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">{db.unused}</div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">File Growth</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">{db.fileGrowth}</div>
 </div>
 </div>

 {/* Tables List */}
 <div className="mt-4">
 <h4 className="text-sm font-semibold text-[var(--foreground)] ">Tables</h4>
 ...
 <span className="font-mono text-sm text-[var(--foreground)] ">{table.size}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 className={`bg-[var(--surface-container)] text-[var(--foreground)] border border-[var(--border)] border-2 rounded-lg p-4 text-left transition-all hover:scale-105 ${
 ...
 <Clock size={18} className={queryFilter === "expensive" ? "text-[var(--primary)] dark:text-[var(--primary)]" : "text-[var(--muted-foreground)]"} />
 <span className={`text-sm font-semibold ${queryFilter === "expensive" ? "text-[var(--primary)] dark:text-[var(--primary)]" : "text-[var(--muted-foreground)]"}`}>
 Most Expensive
 </span>
 </div>
 <p className="text-xs text-[var(--muted-foreground)]">Queries by execution time</p>
 </button>
 ...
 >
 <div className="flex items-center gap-2 mb-1">
 <Activity size={18} className={queryFilter === "frequent" ? "text-[var(--primary)] dark:text-[var(--primary)]" : "text-[var(--muted-foreground)]"} />
 <span className={`text-sm font-semibold ${queryFilter === "frequent" ? "text-[var(--primary)] dark:text-[var(--primary)]" : "text-[var(--muted-foreground)]"}`}>
 Most Frequent
 </span>
 </div>
 <p className="text-xs text-[var(--muted-foreground)]">Queries by execution count</p>
 </button>
 </div>

 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] border border-[var(--border)]  rounded-lg p-4">
 <div className="flex items-center gap-2">
 <Clock size={18} className="text-[var(--primary)] dark:text-[var(--primary)]" />
 <p className="text-sm text-[var(--muted-foreground)]">
 {queryFilter === "expensive" ? "Top 10 queries by execution time" : "Top 10 queries by execution count"} • <span className="font-semibold text-[var(--foreground)] ">{dbType} Environment</span>
 </p>
 </div>
 </div>
 ...
 <div className="flex items-center gap-3">
 <div className="w-8 h-8 rounded-lg bg-[var(--primary)]/20 /20 flex items-center justify-center">
 <span className="text-sm font-bold text-[var(--primary)] dark:text-[var(--primary)]">#{index + 1}</span>
 </div>
 <div>
 <div className="flex items-center gap-2">
 <Clock size={16} className="text-[var(--state-warning)]" />
 <span className="font-mono text-sm font-semibold text-[var(--state-warning)]">{query.executionTime}</span>
 </div>
 <span className="text-xs text-[var(--muted-foreground)]">{query.executionCount.toLocaleString()} executions</span>
 </div>
 </div>
 </div>
 ...
 <div className="p-4 space-y-3">
 {/* Query Text */}
 <div className="bg-[var(--surface-container-high)] text-[var(--foreground)] rounded-lg p-3 overflow-x-auto border border-[var(--border)]  rounded-xl bg-[var(--surface-container-lowest)] dark:bg-[var(--surface-container)]">
 <code className="text-xs text-[var(--state-success)] font-mono whitespace-pre-wrap break-words">
 {query.query}
 </code>
 </div>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Avg Time</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">{query.avgTime}</div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">CPU Time</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">{query.cpuTime}</div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Logical Reads</div>
 <div className="font-mono text-sm font-semibold text-[var(--foreground)] ">{query.logicalReads}</div>
 </div>
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1">Database</div>
 <div className="font-mono text-xs text-[var(--foreground)]  truncate">{query.database}</div>
 </div>
 </div>
 </div>
 </div>
 ...
 <p className="text-sm text-[var(--muted-foreground)]">
 Database Events • <span className="font-semibold text-[var(--foreground)] ">{dbType} Environment</span>
 </p>
 </div>
 </div>
 ...
 className="w-full bg-[var(--surface-container-low)] dark:bg-[var(--card)] px-4 py-3 border-b border-[var(--border)]  flex items-center justify-between cursor-pointer hover:bg-[var(--surface-container-high)] transition-colors"
 >
 <div className="flex items-center gap-3">
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
 event.severity === "High" ? "bg-[var(--state-error-container)] dark:bg-[var(--state-error-container)]" :
 event.severity === "Medium" ? "bg-[var(--state-warning-container)] dark:bg-[var(--state-warning-container)]/30" :
 "bg-[var(--state-warning-container)] dark:bg-[var(--state-warning-container)]"
 }`}>
 <AlertCircle className={`${
 event.severity === "High" ? "text-[var(--state-error)] dark:text-[var(--state-error)]" :
 event.severity === "Medium" ? "text-[var(--state-on-warning-container)] dark:text-[var(--state-warning)]" :
 "text-[var(--state-on-warning-container)] dark:text-[var(--state-warning)]"
 }`} size={18} />
 </div>
 <div className="text-left">
 <div className="flex items-center gap-2">
 <span className="font-semibold text-sm text-[var(--foreground)] ">{event.type}</span>
 <span className={`px-2 py-0.5 rounded text-xs font-bold ${
 event.severity === "High" ? "bg-[var(--state-error-container)] dark:bg-[var(--state-error-container)] text-[var(--state-on-error-container)] dark:text-[var(--state-error)]" :
 event.severity === "Medium" ? "bg-[var(--state-warning-container)] dark:bg-[var(--state-warning-container)]/30 text-[var(--state-on-warning-container)] dark:text-[var(--state-warning)]" :
 "bg-[var(--state-warning-container)] dark:bg-[var(--state-warning-container)] text-[var(--state-on-warning-container)] dark:text-[var(--state-warning)]"
 }`}>
 {event.severity}
 </span>
 </div>
 ...
 <div className="text-xs text-[var(--muted-foreground)]">Database</div>
 <div className="text-sm font-semibold text-[var(--foreground)] ">{event.database}</div>
 </div>
 {isExpanded ? (
 ...
 {/* Event Description */}
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1 font-semibold">Description</div>
 <div className="text-sm text-[var(--foreground)] ">{event.description}</div>
 </div>

 {/* Event Details */}
 <div className="bg-[var(--surface-container-low)] dark:bg-[var(--card)] rounded-lg p-3">
 <div className="text-xs text-[var(--muted-foreground)] mb-1 font-semibold">Details</div>
 <div className="text-xs text-[var(--foreground)] font-mono">{event.details}</div>
 </div>

 {/* Victim/Affected Process */}
 {event.victim && (
 <div className="bg-[var(--state-error-container)] dark:bg-[var(--state-error-container)]/20 border border-[var(--state-error)]/40 dark:border-[var(--state-error)] rounded-lg p-3">
 <div className="text-xs text-[var(--state-on-error-container)] dark:text-[var(--state-error)] mb-1 font-semibold">Deadlock Victim</div>
 <div className="text-sm text-[var(--state-on-error-container)] dark:text-[var(--state-error)] font-mono">{event.victim}</div>
 </div>
 )}
 ...
 <div className="text-xs text-[var(--state-on-info-container)] dark:text-[var(--state-info)] mb-1 font-semibold">Resolution</div>
 <div className="text-sm text-[var(--state-on-info-container)] dark:text-[var(--state-info)]">{event.resolution}</div>
 </div>
 </div>
 )}
 </div>
 ...
 <div className="bg-[var(--surface-container)] text-[var(--foreground)] border border border-[var(--border)]  rounded-xl p-8 text-center">
 <CheckCircle2 size={48} className="mx-auto mb-3 text-[var(--state-success)]" />
 <h3 className="text-lg font-semibold text-[var(--foreground)]  mb-2">No Events Recorded</h3>
 <p className="text-sm text-[var(--muted-foreground)]">
