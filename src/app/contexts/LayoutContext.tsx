...
// Layout Context v3.1 - manages global layout state including sidebar, AI panel, and column visibility
// CACHE BUST: 2024-07-05-v1
...
export function LayoutProvider({ 
  ...
  const getDefaultPinnedItems = (role: string): PinnedItem[] => {
    ...
    const defaultItems: PinnedItem[] = [
      ...
      { id: "dashboards-user-workstation-activity", title: "User Workstation Activity", icon: "Activity", path: "/app/user-workstation-activity" },
      {
        ...
        children: [
          ...
          { id: "workstation-cycle-count", title: "Cycle Count", icon: "Search", path: "/app/cycle-count" },
          { id: "workstation-inspection", title: "Inspection", icon: "PackageSearch", path: "/app/inspection" },
          { id: "workstation-dewrap", title: "DeWrap", icon: "Package", path: "/app/dewrap" },
          { id: "workstation-delayer", title: "Manual DeLayer", icon: "Package", path: "/app/delayer" },
          { id: "workstation-mhe-control-panel", title: "MHE Control Panel", icon: "Power", path: "/app/mhe-control-panel" },
          { id: "workstation-asset-operations", title: "Asset Operations", icon: "Archive", path: "/app/asset-operations" },
          { id: "workstation-mpc-operations", title: "MPC Operations", icon: "ClipboardCheck", path: "/app/mpc-operations" },
        ]
      },
      { 
        id: "operations", 
        ...
        children: [
          ...
          { id: "operations-containers", title: "Containers", icon: "Box", path: "/app/containers" },
          {
            ...
            tertiaryOptions: [
              ...
              { id: "operations-data-analysis-lane-management", title: "Lane Management", icon: "Layers", path: "/app/lane-management" },
  ...
  const [pinnedItems, setPinnedItems] = useState<PinnedItem[]>(() => {
    ...
    try {
      ...
      const currentVersion = "3.1.0"; // Updated version — children with paths required

      // Validate that saved items have correct structure (children with paths)
      let needsReset = false;
      if (saved && version === currentVersion) {
        try {
          const items = JSON.parse(saved);
          // Check if any group items are missing their children
          const hasMissingChildren = items.some((item: PinnedItem) => {
            const isGroup = item.id === 'workstation-operations' || item.id === 'operations';
            return isGroup && (!item.children || item.children.length === 0);
          });
          ...
          if (hasMissingPaths || hasMissingChildren) {
            needsReset = true;
          }
        } catch {
          needsReset = true;
  ...
  const initializePinnedItems = (username: string, role: string) => {
    ...
    const currentVersion = "3.1.0"; // Bumped version — children with paths required
